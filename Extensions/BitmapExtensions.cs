...
namespace Extensions
{
    public static class BitmapExtensions
    {
        ...
        public static Bitmap ToBitmap(this WebBrowser browser)
        {
            using (System.Drawing.Bitmap bm = new System.Drawing.Bitmap(browser.Width, browser.Height))
            {
                browser.DrawToBitmap(bm, new Rectangle(0, 0, browser.Width, browser.Height));
                return bm;
            }
        }
        ...
        //title, uri
        public static Newtonsoft.Json.Linq.JArray GetYoutubeVideos(string search)
        {
            try
            {
                string results = Api.Get("https://bridge.suumitsu.eu/?action=display&bridge=YoutubeBridge&s=" + System.Web.HttpUtility.UrlEncode(search) + "&pa=&format=JsonFormat");
                Newtonsoft.Json.Linq.JArray arr = (Newtonsoft.Json.Linq.JArray)Newtonsoft.Json.JsonConvert.DeserializeObject(results);
                return arr;
            }
            catch (Exception ex)
            {
                Models.Site.LogError(ex);
